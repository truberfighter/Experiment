...
void GameSaver::m_printGlobalVariables(){
	...
	m_stream<<SWAMPS_TO_MAKE<<" "<<DESERTS_TO_MAKE<<" "<<WARMING_EXACERBATION<<" "<< DESERTIFICATION_BOUND <<" "<< HORIZONTAL_HOPPING_DISTANCE<<" "<< DESERTIFICATION_SPREADER<<" "<< TURNS_UNTIL_SWAMPED<<std::endl;
	m_stream<<theGame->m_NationsPlaying().size()<<std::endl;
...
void GameSaver::m_printFields(){
	...
	std::shared_ptr<Field> ff;
	if(static_cast<bool>(f)){
		GameLoader gl(f,fileString);
		ff = gl.m_loadField(f);
	}
}
...
NationJson Nation::m_createJson(){
	...
	std::vector<int> actives, inactives;
	...
	for(std::shared_ptr<Figure> currentFigure: m_figures){
		if(std::find(actives.begin(),actives.end(),currentFigure->m_FigureID())==actives.end())
			inactives.push_back(currentFigure->m_FigureID());
	}
	...
	return NationJson{
(int) m_government, (int) m_nation,m_leaderName, actives,inactives,m_taxRate,m_luxuriesRate,m_treasury,technologies,
(int) m_whatToExplore, m_explorationProgress,(int) m_difficulty,embassies
	};
}
...
CityJson City::m_createJson(){
	...
	return CityJson{
		...
		m_shields,m_food,tradeRoutePartners
	};
}
...
GameJson Game::m_createJson(){
	return GameJson{
m_currentYear.m_yearNumberRaw,m_currentYear.m_randomSeed,m_hasWonderBeenBuilt.size(),m_nationAtCurrentTurnIndex,m_climateState,m_pollutionCount
	};
}
